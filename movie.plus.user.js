...
if (!document.getElementById("seBwhA") && document.title.indexOf('豆瓣') !== -1) {
  ...
  if (location.href.startsWith('https://movie.douban.com/subject/')) {
    ...
    $(document).ready(function () {
      let getDoc, getJSON, parseURL, postDoc;
      ...
      let site_gaoqing, site_sub, update_site_online_sites, update_site_gaoqing_sites, update_site_sub_sites;
      update_site_gaoqing_sites = function (title, en) {
        let link, link_parsed, name, site_gaoqing_sites;
      ...
      update_site_sub_sites = function (title, en) {
        let link, link_parsed, name, site_offline_sites;
        title = encodeURI(title);
        if (en) {
          site_offline_sites = {
      ...
      if (imdb.length) {
        ...
        getDoc(imdb_href, null, function (doc, resp, meta) {
          let i, item, len, metascore, parse, popularity, ratingCount, ratingValue, rating_douban,
